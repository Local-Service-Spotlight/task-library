...
1. **Specific Goal, Content and Targeting in the opening** — make the reader's actual situation, reason to care, useful outcome and supporting mechanism clear in the first two or three sentences at grade 5 or below. Lead with a concrete problem, useful finding or true moment. In those same first sentences, explain one useful connection to the larger process, a prerequisite, or the next task, with a link to its maintained guide when available. The reviewer must quote the relationship and check that the linked page supports it. The body must deliver the promise. Explain unfamiliar terms on their first mention and link the maintained owned explanation. A keyword, grade score or generic audience label cannot approve meaning.
...
- Record task-specific semantic evidence in the existing `build/article-certifications.json`, using the ten explicit checks and separate revision-observation schema in `build/ARTICLE-SEMANTIC-REVIEWS.md`. Those checks cover the current nine article requirements, including the plain what/why/linked-connection opening, verified examples, first-screen visual and phone/desktop layout, Content Factory handoff, CTA, short URL, metadata and evidenced E-E-A-T. The observation must name the same task, exact mapped URL and representation, be at least as new as the review, match its source hash and be no more than 24 hours old at build time. Missing, failed, older or stale observations stay `unknown`; a different observed hash is `unmet`. The build never fetches a live page to renew this evidence. Existing URL-level holds always win.
...
2. **Write the specific GCT opening** in two or three short sentences at grade 5 or below. Explain the reader's situation, why this matters, the useful outcome and how the task helps. Within those first sentences, explain one useful connection to the larger process, a prerequisite, or the next task and link the maintained guide when available. A meaningful task visual may lead. Save the exact opening and quoted reviewer evidence in the existing run receipt; the body must deliver its promise.
...
A review hash records which skill-file bytes were examined; it does not make every instruction requirement pass. The instruction gate also requires explicit verdicts for opening, recipe, links, evidence and handoff. A legacy review without those verdicts cannot supply them. Editing the maintained skill bytes expires that review. The canonical article has a separate revision and review trail; never compare its revision hash with the skill-file hash.

The execution ledger supports optional structured `acceptanceReviews`, described in [the execution schema](EXECUTION-LEDGER.md). A completed label alone cannot pass acceptance: the exact task and recipe revision, current instruction revision, independent reviewer, measured result, handoff evidence and a separate fresh source observation must agree. Missing evidence remains unknown, and a later failed or partial attempt cannot be hidden by an old passing review. Optional structured `setupReviews` now record the tested manual-guide route in that same ledger. A setup pass needs an unassisted novice-human attempt, exact package/member/onboarding and maintained instruction identities, independent file/input/access/result/handoff checks, a matching accepted result and a fresh recipe observation. A fresh-agent or assisted rehearsal remains narrower evidence, never an unassisted novice-human pass. Missing evidence stays unknown; observed failures and holds remain visible. Installation, scheduling and other apps are not certified by this manual-guide check. Keep private proof behind the existing allowlist; do not create a second run or setup registry.
...
- **Reported complete** — the contributor claims that the individual task's SOP/skill is complete and mapped; this task-level status does not by itself certify a shared article hub. A shared article is catalog-ready when every mapped task is reported complete and no reviewed hold remains. A visible Definitive marker also requires the positive exact-revision semantic review described above; catalog readiness alone cannot supply it.
